...
// console.log('getAndLock', arguments);
    ...
    async.map(_array(keys), function __handleMap(key, done) {
        return this._bucket.getAndLock(key, options, function __handleGetAndLock(err, res) {
            // only
            if (err) {
                if (err.code !== cbErrors.keyNotFound) {
            ...
            return setImmediate(done.bind(null, null, res));
        });
...
// console.log('unlock', arguments);
    ...
    _indexCas(keys, tokens);

    async.each(_array(keys), function __handleEach(key, done) {
        this._bucket.unlock(key, tokens.cas[key], done);
    }.bind(this), callback);
};
