...
module.exports  = function () {
    it('should get and touch a single key', function (done) {
        ...
        bucket.getAndTouch('a', 1, function (err, res, misses) {
            throwError(err);
    ...
    it('should get and touch multiple keys', function (done) {
        ...
        bucket.getAndTouch(['a', 'b', 'c'], 1, function (err, res, misses) {
            throwError(err);
    ...
    it('should return undefined when getting and touching a single non-existing key', function (done) {
        bucket.getAndTouch('non-existing-key', 1, function (err, res, misses) {
            throwError(err);
    ...
    it('should return undefined when getting and touching multiple non-existing keys', function (done) {
        ...
        bucket.getAndTouch(['non-existing-key-1', 'non-existing-key-2', 'non-existing-key-3', 'a', 'b', 'c'], 1, function (err, res, misses) {
            throwError(err);
