...
module.exports  = function () {
    it('should counter a single key', function (done) {
        ...
        bucket.counter('counter7', delta, function (err, res, cas, misses) {
            throwError(err);

            expect(misses.length).to.be(0);
            expect(res.counter7).to.be(keys.counter7 + delta);
            expect(cas.counter7).to.be.ok();

            bucket.get('counter7', function (err, res) {
                throwError(err);
    ...
    it('should counter multiple keys', function (done) {
        ...
        bucket.counter(['counter0', 'counter1', 'counter2'], delta, function (err, res, cas, misses) {
            throwError(err);
    ...
    it('should put key in misses when countering single non-existing key', function (done) {
        bucket.counter('non-existing-key', 1, function (err, res, cas, misses) {
            throwError(err);
    ...
    it('should put key in misses when countering multiple non-existing keys', function (done) {
        ...
        bucket.counter(['non-existing-key-1', 'non-existing-key-2', 'non-existing-key-3', 'counter0', 'counter1', 'counter2'], delta, function (err, res, cas, misses) {
            throwError(err);
    ...
    it('should support providing CAS tokens indexed by key', function (done) {
        ...
        bucket.get(['counter0', 'counter1', 'counter2', 'counter5'], function (err, res, cas) {
            ...
            bucket.counter(['counter0', 'counter1', 'counter2'], delta, { cas: cas }, function (err, res, cas, misses) {
                throwError(err);
                ...
                bucket.counter(['counter3', 'counter4', 'counter5'], delta, { cas: cas }, function (err, res, cas, misses) {
                    throwError(err);
    ...
    it('should support providing CAS tokens in the standard API', function (done) {
        ...
        bucket.get('counter0', function (err, cas) {
            ...
            bucket.counter('counter0', delta, { cas: cas.counter0 }, function (err, res, cas, misses) {
                throwError(err);
